                                     ...
                                     GDBusInterfaceInfo *iface)
{
    ...
    for (i = 0; iface->annotations && iface->annotations[i]; i++) {
        if (g_str_equal(iface->annotations[i]->key, 
                        "org.isoftlinux.OSProber.VendorExtension")) {
                                ...
                                const gchar *path)
{
    ...
    while ((name = g_dir_read_name(dir))) {
         ...
         * private to isoftosprober and point it at the file,
        ...
        if (!symlink) {
            g_warning("Found isoft os-prober vendor extension file %s, but file "
        ...
        if (g_str_has_prefix(symlink, prefix) && 
            ...
            g_warning("Found isoft os-prober vendor extension symlink %s, but it "
